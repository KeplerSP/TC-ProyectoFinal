...
```
### 3.2 Simulador AFND — Explicación paso a paso

1. Se inicia con el conjunto `{q0}`.
2. Para cada símbolo de la cadena:
   - Se calcula el conjunto de estados destino para todos los estados actuales.
   - El nuevo conjunto es el resultado de unir todas esas transiciones.
3. Si al finalizar, el conjunto contiene al menos un estado de aceptación → **cadena aceptada**.
...
def simular(self, cadena):
        ...
        for simbolo in cadena:
            ...
            for estado in actuales:
                if (estado, simbolo) in self.trans:
                    nuevos |= self.trans[(estado, simbolo)]
        ...
        return any(s in self.aceptacion for s in actuales)
```
### 3.3 Construcción de Subconjuntos

### Cada estado del AFD corresponde a un conjunto de estados del AFND. Partiendo del {q0}, exploramos transiciones por cada símbolo y producimos nuevos conjuntos hasta no hallar más.
```js
 def convertir_a_afd(self):

        inicial_afd = frozenset([self.inicial])

        por_procesar = [inicial_afd]
        procesados = set()
        trans_afd = {}        # transiciones del AFD
        acept_afd = set()     # estados de aceptación del AFD
        ...
        while por_procesar:
            ...
            # verificar si este estado del AFD es de aceptación
            if any(s in self.aceptacion for s in S):
                acept_afd.add(S)

            # calcular transiciones δ'(S,a)'
            for simbolo in self.alfabeto:
                ...
                trans_afd[(S, simbolo)] = destino

                # si aparece un nuevo conjunto, agregar para procesar
                if destino not in procesados and destino not in por_procesar:
